...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      throw new Error(`钙钛矿参数预测API响应错误: ${response.status} ${response.statusText}`);
    }

    const data = await response.json();
    console.log('钙钛矿参数预测响应:', data);

    return NextResponse.json(data);

  } catch (error) {
    console.error('钙钛矿参数预测API错误:', error);
    
    const errorMessage = error instanceof Error ? error.message : '未知错误';
    
    return NextResponse.json({
      success: false,
      error: `参数预测失败: ${errorMessage}`,
