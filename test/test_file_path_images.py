...
TIMEOUT = 120  # 设置超时时间，单位为秒

def test_file_path_image_sending():
    """测试基于file_path的图片发送功能"""
    logger.info("开始测试基于file_path的图片发送功能")
    
    # 创建一个新的会话
    session_id = f"test_file_path_session_{int(time.time())}"
    ...
    try:
        ...
        image_count = 0
        text_count = 0
        save_dir = "test_file_path_images"
                ...
                if line.startswith(b"data: "):
                    ...
                    try:
                        ...
                        if data.get("type") == "image":
                            ...
                            source_path = image_data.get("source_path", "unknown")
                            ...
                            filename = f"{save_dir}/{tool_name}_file_path_image_{image_index}_{timestamp}.{image_format}"
                            
                            with open(filename, "wb") as f:
                                f.write(img_bytes)
                                
                            logger.info(f"成功接收并保存图像: {filename}")
                            logger.info(f"图像源文件路径: {source_path}")
                        else:
                            text_count += 1
                            # 仅记录完成消息
...
def main():
    ...
    for attempt in range(1, max_retries + 1):
        ...
        result = test_file_path_image_sending()
        if result:
            logger.info("✅ 基于file_path的图片发送测试成功")
            return
        elif attempt < max_retries:
            logger.warning(f"测试失败，等待5秒后重试...")
            time.sleep(5)
    
    logger.error("❌ 基于file_path的图片发送测试失败（已尝试多次）")
